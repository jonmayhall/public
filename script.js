...
document.addEventListener("DOMContentLoaded", function () {
  ...
  pagesWithStatus.forEach((page) => {
    ...
    const updateStatus = () => {
      const inputs = [...page.querySelectorAll("input, select")].filter(
        (el) => !el.closest(".section-block.comment-box")
      );
  ...
  const dealerInput = document.querySelector(
    "#dealership-info input:nth-of-type(2)"
  );
  const groupInput = document.querySelector(
    "#dealership-info input:nth-of-type(1)"
  );
  ...
  const updateHeader = () => {
    ...
    if (
      !group ||
      /^(n\/a|none|unknown|i\s?don.?t\s?know)$/i.test(group)
    )
      headerDisplay.textContent = dealer;
    else headerDisplay.textContent = `${group} - ${dealer}`;
  };
  ...
  const saveBtn =
    ...
    document.getElementById("submitSummaryBtn");
  ...
  async function submitToSheet() {
    ...
    document.querySelectorAll(".page-section").forEach((page) => {
      ...
      const fields = [...page.querySelectorAll("input, select, textarea")].map(
        (el) => ({
          name: el.placeholder || el.previousSibling?.textContent || "",
          value: el.value,
        })
      );
  ...
  if (summary) {
    ...
    const fillSummary = () => {
      Object.entries(fillMap).forEach(([id, sectionHeader]) => {
        ...
        if (srcPage && destBox) {
          const notes = [
            ...srcPage.querySelectorAll(".section-block.comment-box textarea"),
          ]
            .map((t) => t.value.trim())
            .filter(Boolean)
            .join("\n\n");
    ...
    document
      ...
      .forEach((t) => t.addEventListener("input", fillSummary));
    fillSummary();
  }
});
