...
function colorSelect(sel) {
  ...
  sel.style.backgroundColor = colors[v] || "#f0f0f0";
}
...
// === Dynamically Build Training Tables ===
const roles = [
...
roles.forEach(role => {
  ...
  div.classList.add("table-section");
  div.innerHTML = `
    <div class="section-header">${role.title}</div>
    <div class="table-container">
      <table id="${role.id}" class="training-table">
        <thead>
          <tr><th style="width:240px;">Name</th>${role.cols.map(c => `<th>${c}</th>`).join("")}</tr>
        </thead>
        <tbody>
